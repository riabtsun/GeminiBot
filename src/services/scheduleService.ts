...
import { MyContext } from "../bot"; // Наш кастомный контекст
...
export class ScheduleService {
    ...
    try {
      ...
      activeUsers.forEach((user) => {
        ...
        if (this.validateUserData(user)) {
          ...
          console.warn(
            `Пользователь <span class="math-inline">\{user\.telegramId\} \(</span>{user.firstName}) имеет невалидные данные времени или таймзоны. Задачи не запланированы.`
          );
  ...
  public scheduleUserTasks(user: IUser) {
    ...
    // Проверяем валидность еще раз на всякий случай
    if (!this.validateUserData(user)) {
      console.warn(
        `Повторная проверка: Пользователь <span class="math-inline">\{user\.telegramId\} \(</span>{user.firstName}) имеет невалидные данные. Задачи не запланированы.`
      );
      return;
    }
